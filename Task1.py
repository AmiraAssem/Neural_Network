...
class Perceptron:
    def __init__(self, learning_rate=0.01, numberOfIteration=100, isBios=False):
    ...
    def fit(self, X, y):
        ...
        for index in range(self.numberOfIteration):
            for i, x_i in enumerate(X):
                ...
                if self.isBios:
                    y_predicted = self.activationFunction(netValue + self.bias)
                    error = (y[i] - y_predicted)
                    self.weights += self.learning_rate * error * x_i
                    self.bias += self.learning_rate * error
                else:
                    y_predicted = self.activationFunction(netValue)
                    error = (y[i] - y_predicted)
                    self.weights += self.learning_rate * error * x_i
        self.plot_LinearPreception(inputs=X, weights=self.weights, bias=self.bias)
    ...
    def plot_LinearPreception(self, inputs, weights, bias):
...
def Call_back(features, classes, learning_rate, epochs, isBias):
    ...
    p = Perceptron(learning_rate=float(learning_rate), numberOfIteration=int(epochs), isBios=isBias)
    p.fit(X_train, y_train)

    predictions = p.predict(X_test)
    predictions1 = p.predict(X_train)

    print(confusion_matrix(y_test, predictions))

    return p.accuracy_score(y_test, predictions), p.accuracy_score(y_train, predictions1)
