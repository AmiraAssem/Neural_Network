...
mse = tk.StringVar()
epochs_entry = ttk.Entry(window, textvariable=mse, font=("Times New Roman", 10))
epochs_entry.grid(column=1, row=25)
...
def Process():
    acc = Task2.Call_back(str(Feature_choice.get()), str(Class_choice.get()),
                          learning_rate.get(), epochs.get(), mse.get(), IsBias.get())
